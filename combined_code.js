...
export const ChartCanvas: React.FC<ChartCanvasProps> = ({
    ...
    .
        ...
        :
            () => {
                const dx = x - startPoint
                !
            .
                x;
                const dy = y - startPoint
                !
            .
                y;
...
?:
    ...
    const
    logEntry = `[${timestamp}] ${message}${data ? '\n' + JSON.stringify(data, null, 2) : ''}`;
    this
.
    logs
.
...
void {
    ...
    ctx
    .lineTo(this.toX - headLength * Math.cos(angle - Math.PI / 6), this.toY - headLength * Math.sin(angle - Math.PI / 6));
    ctx
    .lineTo(this.toX - headLength * Math.cos(angle + Math.PI / 6), this.toY - headLength * Math.sin(angle + Math.PI / 6));
    ctx
    .lineTo(this.toX, this.toY);
...
'./styles/Toolbar';
@use
'./styles/SettingsToolbar';
@use
'./styles/Overlays';
@use
'./styles/Canvas/Axes';
@use
'./styles/Canvas/ChartCanvas';
@use
'./styles/Canvas/ChartStage';
@use
'./styles/Canvas/Grid';
...
export
type
ChartOptions = LineChartOptions | CandleChartOptions;
